...
import { WorkerPool, type WorkerPoolStats } from "./WorkerPool";
...
describe("WorkerPool", () => {
	...
	type StatsType = WorkerPoolStats;
	...
	test("combines maxTasksPerWorker and maxWorkerLifetimeMs", async () => {
		...
		// The lifetime must force a replacement. On a slow machine, either of the
		// first two calls may also cross the deadline, but no call can create more
		// than one worker.
		expect(workerCreationCount).toBeGreaterThanOrEqual(2);
		expect(workerCreationCount).toBeLessThanOrEqual(3);
	...
	test("worker marked for termination after maxTasksPerWorker with concurrent tasks", async () => {
		...
		expect(workerTerminationCount).toBe(2); // Both workers reach their strict task limit
	...
	// --- Bug Reproduction: Workers Stop Spawning ---

	test("workers continue spawning after termination with queued tasks", async () => {
		const stats: StatsType[] = [];
		const pool = new WorkerPool({
			size: 4,
			maxTasksPerWorker: 1, // Terminate after each task
			workerFactory: createTestWorker,
			proxyFactory: comlinkProxyFactory,
			onUpdateStats: (s) => stats.push({ ...s }),
		});
		...
		// Queue 8 tasks that will require worker recycling
		const tasks = Array.from({ length: 8 }, (_, i) =>
			api.delayAndReturn(50, `task${i}`),
		);

		// Wait a bit to let some workers get marked for termination
		await new Promise((resolve) => setTimeout(resolve, 100));

		// Check stats during execution - should never have workers that are neither idle nor running tasks
		const problematicStats = stats.filter(
			(s) =>
				s.workers > 0 &&
				s.idleWorkers === 0 &&
				s.runningTasks === 0 &&
		...
		// Should not have any problematic stats where workers exist but aren't processing
		expect(problematicStats.length).toBe(0);
